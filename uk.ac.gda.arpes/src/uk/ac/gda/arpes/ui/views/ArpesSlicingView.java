...
public class ArpesSlicingView extends ViewPart implements IObserver {
	...
	private IObservable dataDispatcher;
	...
	private DoubleDataset getScanAxis(String scanCommand) {
		...
		try {
			...
			final double step = start<stop? Math.abs(Double.parseDouble(scanCommandArray[4])):(-1.0)*Math.abs(Double.parseDouble(scanCommandArray[4]));
