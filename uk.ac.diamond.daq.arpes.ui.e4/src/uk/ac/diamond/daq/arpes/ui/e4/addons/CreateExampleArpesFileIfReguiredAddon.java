...
public class CreateExampleArpesFileIfReguiredAddon {
	// Because of the shared nature of Editor across E4 perspectives I essentially just need to create ArpesFile
	// one time only when one of those persepctives start up and then only recreate if all of them were closed.
	private static final Logger logger = LoggerFactory.getLogger(CreateExampleArpesFileIfReguiredAddon.class);
	private boolean isExampleArpesFileOpen = false;
	...
	private void checkCreateExampleArpesFile(MPerspective perspective) {
		if (isExampleArpesFileOpen){
			logger.debug("CreateExampleArpesFile was already called on other perspective");
			return;
		}
		if (SET_OF_PERSPECTIVES.stream().anyMatch(name -> name.contains(perspective.getElementId()))) {
			logger.debug("CreateExampleArpesFile called");
			createExampleArpesFileIfRequired();
			isExampleArpesFileOpen = true;
		}
	}
