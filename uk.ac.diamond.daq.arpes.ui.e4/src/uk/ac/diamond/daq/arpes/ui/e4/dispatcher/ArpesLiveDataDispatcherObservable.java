...
public class ArpesLiveDataDispatcherObservable extends FindableConfigurableBase implements IObservable {
	private static final Logger logger = LoggerFactory.getLogger(ArpesLiveDataDispatcherObservable.class);
	...
	public void configure() throws FactoryException {
		...
		try {
			...
			// set monitor on acquisition mode - need manufacturer as Scienta AcqMode enums are opposite to MBS
	...
	private void emitNewData() {
		try {
			...
			//This bit is for Slicing view or any view that wants to get progress on iterations
			if (monitorIterationProgress){
	...
	private IDataset getXAxis() throws Exception {
		double[] xdata = epicsController.cagetDoubleArray(getChannel(getAnalyserEnergyAxisPv()), epicsController.cagetInt(getChannel(getAnalyserEnergyAxisCountPv())));
	...
	private IDataset getYAxis() throws Exception {
		double[] ydata = epicsController.cagetDoubleArray(getChannel(getAnalyserAngleAxisPv()), epicsController.cagetInt(getChannel(getAnalyserAngleAxisCountPv())));
	...
	private void setAcquisitionMode(final MonitorEvent event) {
		...
		// Scienta epics enums for acquisition mode are reversed to AcquisitionMode: [ 0] Swept [ 1] Fixed
		acquisitionMode = analyserManufacturer.contains("Scienta")?
				AcquisitionMode.values()[1-enumeration.getEnumValue()[0]]:AcquisitionMode.values()[enumeration.getEnumValue()[0]];
