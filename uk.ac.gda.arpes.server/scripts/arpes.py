...
class ARPESRun:
	...
	def run(self):
		...
		# Set the pass energy
		logger.debug("Setting pass energy")
		self.analyser.setPassEnergy(self.bean.getPassEnergy())
		# Set the lens mode
		logger.debug("Setting lens mode")
		...
		if self.bean.getAcquisitionMode() == uk.ac.diamond.daq.pes.api.AcquisitionMode.SWEPT:
			# Exact sequence of calls to set region is IMPORTANT here!
			logger.debug("Setting energy step")
			self.analyser.setEnergyStep(self.bean.getStepEnergy()/1000.0)

			logger.debug("Setting number of steps")
			self.analyser.setNumberOfSteps(self.bean.getNumberOfSteps())

		logger.debug("Setting centre energy")
		...
		if isinstance(self.analyser, uk.ac.diamond.daq.pes.api.IDeflector):
			logger.debug("Setting deflector x")
