 ...
 */
package uk.ac.diamond.daq.configuration.commands;
...
public class ObjectFactoryCommand implements ServerCommand {
	...
	public void execute() throws FactoryException, IOException {
		SpringContext context = new SpringContext(xmlFiles, profiles);
	...
	private void addScriptProject(File file) {
		...
		try {
			...
			throw new IllegalStateException(
					"Unable to get Jython Server, cannot add " + file.getName() + " to script projects.", exception);
		}
		...
		scriptPaths.addProject(
				new ScriptProject(file.getParent(), "Scripts: " + file.getName(), ScriptProjectType.HIDDEN));
	}
