...
public abstract class AbstractBoundingLineModel extends AbstractMapModel implements IBoundingLineModel, IBoundsToFit {
	 ...
	 * @return pointsModel - a TwoAxisLinePointsModel that will output IPositions that very closely match the IPositions generated by the input
	 */
	public static TwoAxisLinePointsModel enforceShape(TwoAxisLineStepModel model) {
		...
		if (original != null) {
			...
			final double step = IBoundsToFit.getLongestFittingStep(copy.getLength(), model.getStep(), model.isBoundsToFit());
			int points = IBoundsToFit.getPointsOnLine(copy.getLength(), step, model.isBoundsToFit());
			pointsModel.setPoints(points);
			if (!model.isBoundsToFit() && Math.abs(step) > Math.abs(copy.getLength())) {
				// Step > Length, !boundsFit: We extends the line to match the step.
				copy.setLength(step * Math.signum(copy.getLength()));
			} else {
				...
				copy.setLength(step * ((model.isBoundsToFit() || Math.abs(points) == 1 ) ? points : (int) (points - Math.signum(points))));
			}
		}
