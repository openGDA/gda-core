...
/**
 * Extracts from the SavedScanMetaData a value for various details for a {@link Browser} column.
 *
 * @author Maurizio Nagni
 */
class DetailsProvider extends LabelProvider implements IComparableStyledLabelProvider {
	 ...
	 */
	@Override
	public StyledString getStyledText(Object el) {
		StyledString result = new StyledString();
		DecimalFormat decimal = new DecimalFormat("#########.###");
		SavedScanMetaData element = MapBrowser.extractSavedScanMetaData(el);
		...
		if (tokens.length > 2) { // if there is a descriptor section
			...
			if (descriptor.startsWith("S0")) {
				...
				result.append(
						PathParameterSource.getSummary(dimensions[PATH_TYPE], dimensions[PATH_PARAMS].split(",")[0],
								RegionParameterSource.getSummary(dimensions[SHAPE_TYPE],
