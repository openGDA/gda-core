...
public class GainCalculation {
	...
	public static double getIntensity(final GainBean bean) throws Exception {
		...
		while (monocromator.isBusy()&&!bean.isCancelled()) Thread.sleep(1000);
		if (bean.isCancelled()) {
		...
		if (!DoubleUtils.equalsWithinTolerance((Double)monocromator.getPosition(),bean.getEnergy(),0.1)) {
			throw new Exception("The mononchromator scannable is value '"+monocromator.getPosition()+"' and should be "+bean.getEnergy());
		}
	...
	private static String getOptimalGainByReducing(final GainBean bean, final String curGain) throws Exception {
        ...
        if (index==0) {
        	final IntensityException e = new SaturationException("The intensity is saturating at minimum gain '"+NOTCHES.get(0)+"' for chamber '"+bean.getIonChamberName()+"'.\nThis means there is a problem with the experimental set up.\n\nPlease contact a member of beam-line staff or try a different amplifier.");
            bean.log("Error - The intensity is saturating at minimum gain '"+NOTCHES.get(0)+"' for chamber '"+bean.getIonChamberName()+"'.");
	...
	private static String getOptimalGainByIncreasing(final GainBean bean, final String curGain) throws Exception {
		...
		try {
        	...
        	final IntensityException e1 = new SmallIntensityException("The intensity is small at the maximum gain setting of '"+NOTCHES.get(NOTCHES.size()-1)+"' for chamber '"+bean.getIonChamberName()+"'.\n\nThe maximum value will be set for this chamber.");
            bean.log("Warning - The intensity is small at the maximum gain setting of '"+NOTCHES.get(NOTCHES.size()-1)+"' for chamber '"+bean.getIonChamberName()+"'.");
	...
	private static double getIntensityAtGain(final GainBean bean, final String gain) throws Exception {
        ...
        bean.log(bean.getIonChamberName()+"\t\t\t"+bean.getCurrentAmplifierName()+"\t"+gain+"\t"+bean.getEnergy()+"\t\t"+intensity);
