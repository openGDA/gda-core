...
public class EdgePlotSelectionListener implements Overlay1DConsumer {
	...
	private volatile double latestTargetValue = 0;
	...
	private void doMouseMoved(final AreaSelectEvent event) {
		if (!editor.suspendGraphUpdate) {
			doMousePressed(event);
		} else {
			final double[] lines = editor.edgeOverlay.xValues;
			
			if (newValueAcceptable(lines,event.getX())) {
				
				latestTargetValue = event.getX();
				...
				editor.getSite().getShell().getDisplay().asyncExec(new Runnable() {
					...
					protected double calcGaf1() {
						final double ed = editor.getEdgeValue();
						final double coreHole = Double.parseDouble(editor.getCoreHole_unused().getValue());
						double gaf1 = Math.round((ed - latestTargetValue) / coreHole);
						return gaf1;
					}

					protected double calcGaf2() {
						final double ed = editor.getEdgeValue();
						final double coreHole = Double.parseDouble(editor.getCoreHole_unused().getValue());
						double gaf2 = Math.round((ed - latestTargetValue) / coreHole);
						return gaf2;
					}

					protected double calcGaf3() {
						final double ed = editor.getEdgeValue();
						final double coreHole = Double.parseDouble(editor.getCoreHole_unused().getValue());
