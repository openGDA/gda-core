...
public class ImportXspress3ROIWizardPage extends ImportROIWizardPage {
	...
	private void updateAddButtonEnables() {
		...
		} else if (getBeansToAdd().size() >= Xspress3ParametersUIHelper.INSTANCE.getMaximumRegions()) {
	...
	public class DetectorComposite extends Composite {
		...
		public DetectorComposite(Composite parent, int style, double maximum) {
			...
			regionList.setMinItems(Xspress3ParametersUIHelper.INSTANCE.getMinimumRegions());
			regionList.setMaxItems(Xspress3ParametersUIHelper.INSTANCE.getMaximumRegions());
			regionList.addValueListener(new ValueAdapter("Xspress3 Region List Listener") {
				@Override
