...
//		getNdFile().setFileWriteMode(FileWriteMode.STREAM);
	...
	protected NXDetectorDataAppender createFileLinkAppender() {
		...
		NXDetectorDataFileLinkAppender nxDetectorDataFileLinkAppender = new NXDetectorDataFileLinkAppender(expectedFullFileName, getxPixelSize(), getyPixelSize(), getxPixelSizeUnit(), getyPixelSizeUnit(),dataRank);
		...
		if (collection.isPresent()) {
			...
			appenders.add(nxDetectorDataFileLinkAppender);
	...
	protected List<NXDetectorDataAppender> createAppendersFromCollection(EpicsProcessVariableCollection collection) {
		List<NXDetectorDataAppender> appenders = new ArrayList<>();
		INexusTree pvCollectionNode = collection.createPVCollectionNode(collection.getName());
		NXDetectorDataChildNodeAppender nxDetectorDataChildNodeAppender = new NXDetectorDataChildNodeAppender(pvCollectionNode);
		appenders.add(nxDetectorDataChildNodeAppender);
		Map<String, String> detectorData = collection.createFieldsToAppend();
		NXDetectorDataStringAppender nxDetectorDataStringAppender = new NXDetectorDataStringAppender(List.copyOf(detectorData.keySet()), List.copyOf(detectorData.values()));
		appenders.add(nxDetectorDataStringAppender);
		return appenders;
	}

	protected EpicsProcessVariableCollection pvsToCollectMap;

	public EpicsProcessVariableCollection getPvsToCollectMap() {
		return pvsToCollectMap;
	}

	public void setPvsToCollectMap(EpicsProcessVariableCollection pvsToCollectMap) {
		this.pvsToCollectMap = pvsToCollectMap;
	}
