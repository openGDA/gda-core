 ...
 */
public class AsynPut2 {
	private static final Logger logger = LoggerFactory.getLogger(AsynPut2.class);
	...
	private void onPutCompleted() {
		/*
		 * Channel ch = (Channel)ev.getSource(); Context ctxt = ch.getContext(); // int count = ev.getCount(); //
		 * CAStatus st = ev.getStatus(); // DBRType type = ev.getType(); try { String[] value =
		 * ((STRING)ch.get(DBRType.STRING,1)).getStringValue(); ctxt.pendIO(timeout); Message.debug("The value of " +
		 * this.name + " is now at " + value[0]); } catch (Exception e) { System.err.println("Get failed for " +
		 * this.name + ":\n" + e); }
		 */
	...
	private void onConnectionChanged(ConnectionEvent ev) {
		...
		if (connectionCounter == 0 && ch.getConnectionState() == Channel.CONNECTED) {
			...
			try {
				...
				nameFound = true;
				// Message.debug(JCAUtils.timeStamp() + " Search successful
				// for: " +
				// getName());
				// Add a monitor listener
				ch.addMonitor(DBRType.STRING, 1, Monitor.VALUE | Monitor.ALARM, new JCAMonitorListener());
				ctxt.pendIO(timeout);
			} catch (Exception ex) {
		...
		if (ch.getConnectionState() == Channel.CONNECTED) {
			// Message.debug(JCAUtils.timeStamp() + " " + ch.getName() + "
			// is
			// connected");
			connectionCounter++;
			// Put the value to the channel
			try {
				ch.put(this.value, new JCAPutListener());
				ctxt.pendIO(timeout);
			} catch (Exception ex) {
				System.err.println("Put failed for " + this.name + ":\n" + ex);
				System.exit(1);
			}
			// Message.debug("The value of " + this.name + " is set to " +
			// this.value);
		} else if (ch.getConnectionState() == Channel.CLOSED) {
			logger.debug(JCAUtils.timeStamp() + " " + ch.getName() + " is closed");
		} else if (ch.getConnectionState() == Channel.DISCONNECTED) {
			logger.debug(JCAUtils.timeStamp() + " " + ch.getName() + " is disconnected");
		} else if (ch.getConnectionState() == Channel.NEVER_CONNECTED) {
			logger.debug(JCAUtils.timeStamp() + " " + ch.getName() + " is never connected");
		} else {
			// Shouldn't happen
