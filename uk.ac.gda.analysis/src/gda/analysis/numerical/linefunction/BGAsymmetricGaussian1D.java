 ...
 */
public class BGAsymmetricGaussian1D extends AbstractFunction {
	private static String[] parameterNames = { "area", "position", "sigma1", "sigma2", "background" };
	 ...
	 *            The parameters
	 */
	public BGAsymmetricGaussian1D(double... parms) {
		...
		if (parms.length == 0) {
			...
			getParameter("sigma1").setValue(1.0);
			// Set sigma to 1.0 by default
			getParameter("sigma2").setValue(1.0);
			// Set sigma to 1.0 by default
		...
		getParameter("sigma1").setLowerLimit(0.0);
		getParameter("sigma2").setLowerLimit(0.0);
	...
	public double val(double... positions) {
		...
		double sigma1 = getParameter("sigma1").getValue();
		double sigma2 = getParameter("sigma2").getValue();
		double back = getParameter("background").getValue();

		if (positions[0] <= pos) {
			arg = (positions[0] - pos) / sigma1;
			ex = Math.exp(-0.5 * arg * arg);
		} else {
			arg = (positions[0] - pos) / sigma2;
			ex = Math.exp(-0.5 * arg * arg);
		}
		double norm = Math.sqrt(2.0 * Math.PI) * 0.5 * (sigma1 + sigma2);
		y = back + (area * ex / norm);
