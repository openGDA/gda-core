...
public class GaussianFitter implements Serializable {
	 ...
	 */
	public static List<Double> fitGaussian1D(DoubleDataset data, int width, int step){
	 ...
	 */
	public static List<FunctionOutput> fitGaussianTo1DDataset(DoubleDataset data, int width, int step) {
		if (data.getRank() != 1)
			return null;
		...
		double totalArea = data.getSize() * data.peakToPeak().doubleValue();
		...
		for (int i = 0; (i + step) < data.getSize(); i += step) {
			...
			try {
				...
				funcOut = nelderMeadFit(new DoubleDataset[] {xAxis}, sliceData, 0.0001,
		...
		for (int i = 0; i < allPeaks.size(); i++) {
			IParameter[] params = allPeaks.get(i).getFunction().getParameters();
...
//		}
	 ...
	 * @throws Exception
	 */
	private static FunctionOutput nelderMeadFit(DoubleDataset[] coords, DoubleDataset data,
		...
		result.setChiSquared(comp.residual(true, data, null, coords));
	...
	private static FunctionOutput geneticAlgorithmFit(DoubleDataset[] coords, DoubleDataset data,
			double accuracy, AFunction... functions) throws Exception {
	 ...
	 */
	public static List<Double> fitGaussian2D(DoubleDataset data, double width, double max, double[] pos) {
	 ...
	 */
	public static List<FunctionOutput> fitGaussianTo2DDataset(DoubleDataset data, double width, double max, double[] pos) {
		if (data.getRank() != 2)
			return null;
...
//				funcOut = nelderMeadFit(new DataSet[] {c0, c1}, data, 0.0001, g, new Offset(median/10.,median));
				funcOut = geneticAlgorithmFit(new DoubleDataset[] {c0, c1}, data, 0.1, g, new Offset(median/100.,median));
