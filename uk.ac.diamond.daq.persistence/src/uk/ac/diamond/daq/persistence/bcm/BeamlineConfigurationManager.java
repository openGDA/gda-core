...
import uk.ac.diamond.daq.persistence.jythonshelf.LocalPersistence;
...
public class BeamlineConfigurationManager implements Findable, Configurable, Localizable, IObservable {
	...
	public Collection<String> getAllScannableNames() {
		Set<String> allScannables = new HashSet<>();
	...
	public Collection<String> getAllScannableNames(String modeName) throws BcmException {
		if (modeName == null)
			throw new NullModeBCMException();
		if (activeModes.get(modeName) == null)
			throw new InactiveModeBcmException(modeName);
	...
	private Mode getMode(String modeName) {
		return em.find(Mode.class, modeName);
	}
	...
	public void status() throws DeviceException {
		...
		for (Map.Entry<String, Mode> entry : activeModes.entrySet()) {
			Util.printTerm("Mode " + entry.getKey());
			entry.getValue().reportNonNominalPositions();
			entry.getValue().reportViolatedLimits();
		}
	}
	...
	private final class NullModeBCMException extends BcmException {
		...
		protected NullModeBCMException() {
			super("mode name null pointer.");
		}
	}
