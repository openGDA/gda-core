 ...
 * detectors/scannables to be configured correctly to generate the required number of frames. Note that detectors that
 * support gda.device.detector.DetectorFrameController would normally support
 * gda.device.scannable.PositionCallableProvider as well as frame advance would not normally be controlled by the scan
 * command.
 *
...
public class RepeatScan {
	...
	private static final class NumberOfFramesScannable extends ScannableBase {

		private int repeatCount = 0;
		private List<Scannable> allScannables = null;
		private List<Detector> allDetectors = null;
		...
		public void atScanLineStart() throws DeviceException {
			...
			if (allDetectors != null) {
				for (Detector det : allDetectors) {
					if (det instanceof RepScanScannable repScan) {
						repScan.atRepScanStart(repeatCount);
					}
				}
			}
			if (allScannables != null) {
				for (Scannable scannable : allScannables) {
					if (scannable instanceof RepScanScannable repScan) {
						repScan.atRepScanStart(repeatCount);
					}
				}
			}
	...
	private static final class FrameProvider implements ScanPositionProvider {
