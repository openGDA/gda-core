...
// based on sun's DisplayModeTest.java
...
/**
 * This test generates a table of all available display modes, enters full-screen mode, if available, and allows you to
 * change the display mode. The application should look fine under each enumerated display mode. On UNIX, only a single
 * display mode should be available, and on Microsoft Windows, display modes should depend on direct draw availability
 * and the type of graphics card.
 */
/**
 *
 */
public class ScreenDisplayModeTest extends JFrame implements ActionListener, ListSelectionListener {
	 ...
	 * @param device
	 */
	public ScreenDisplayModeTest(GraphicsDevice device) {
		super(device.getDefaultConfiguration());
		this.device = device;
		setTitle("Display Mode Test - screen" + device.getIDstring());
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < devices.length; i++) {
			ScreenDisplayModeTest test = new ScreenDisplayModeTest(devices[i]);
...
class DisplayModeModel extends DefaultTableModel {
	...
	public String getColumnName(int c) {
		return ScreenDisplayModeTest.COLUMN_NAMES[c];
	}

	@Override
	public int getColumnCount() {
		return ScreenDisplayModeTest.COLUMN_WIDTHS.length;
	}
	...
	public Object getValueAt(int rowIndex, int colIndex) {
		...
		switch (colIndex) {
		case ScreenDisplayModeTest.INDEX_WIDTH:
			return Integer.toString(dm.getWidth());
		case ScreenDisplayModeTest.INDEX_HEIGHT:
			return Integer.toString(dm.getHeight());
		case ScreenDisplayModeTest.INDEX_BITDEPTH: {
		...
		case ScreenDisplayModeTest.INDEX_REFRESHRATE: {
