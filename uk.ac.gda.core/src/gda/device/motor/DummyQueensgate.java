...
/**
 * A class which pretends to be the serial connection to a real Queensgate controller.
 */
public class DummyQueensgate extends MotorBase {
	...
	// private QueensgateController queensgateController;
	private DummyQueensgateController queensgateController;
	 ...
	 */
	public DummyQueensgate() {
	}

	@Override
	public void configure() {
		logger.debug("Real Queensgate would be finding: " + queensgateControllerName);
		try {
			// Allocate dummy QG Controller, instead of finding it
			// queensgateController = new DummyQueensgateController();
			// Get singleton instance of controller, instead of finding it
			queensgateController = DummyQueensgateController.getInstance();
			...
			logger.error("DummyQueensgate: Exception caught while configuring settings" + e);
		}
	}
	...
	private void checkAndConvertReply(String reply) throws MotorException {
		...
		if (Math.abs(positionToSend - position) < 4) {
			position = positionToSend;
		} else {
			// FIXME this needs testing to ensure errors are not thrown
			// unnecessarily. Message statement added for monitoring
			// purposes.
			logger.error("Offset reached does not correspond to offset requested");
			status = 1;
		}
