...
//			df.setTimeZone(tz);
	...
	/**
	 * Returns the initial file structure for this detector including all of its plugins.
	 * This is used if property {@code NexusScanDataWriter#PROPERTY_NAME_CREATE_FILE_AT_SCAN_START} is true to
	 * allow this detector to still be compatible with data writing.
	 */
	@Override
	public NexusTreeProvider getFileStructure() throws DeviceException {
		...
		//Used for data that doesn't create NXdata links, replicates same file structure without creating data before first point.
		List<INexusTree> secondaryData = new ArrayList<>();

		NXPluginBase primaryPlugin = getCollectionStrategy();
		for (NXPluginBase plugin : getPluginList()) {
			logger.debug("Creating file structure for plugin \"{}\"", plugin.getName());
			for (String inputName : plugin.getInputStreamNames()) {
				final INexusTree nexusTree = detectorData.addData(getName(), inputName, new NexusGroupData(0.0), null);

				if (!plugin.getName().equals(primaryPlugin.getName())) {
					secondaryData.add(nexusTree);
				}
				logger.debug("Created empty data for \"{}\"", inputName);
			}
		}
		logger.debug("Finished setting up file structure for plugins.");

		//Replicate file structure that NXDetector gives in getPositionCallable()
		if (addCollectTimeMs) {
			secondaryData.add(detectorData.addData(getName(), TIME_MS, new NexusGroupData(0.0), "ms"));

			INexusTree startTimeData = detectorData.addData(getName(), START_TIME, new NexusGroupData(0.0), "s");
			...
			logger.debug("addCollectTimeMs is true, created initial data for {}, {}, and {}", START_TIME, START, TIME_MS);
		}

		secondaryData.stream().forEach(
			//This is needed because addData will set it to true. This needs to be false to keep data format consistent between scans.
			//In a regular scan (file created at first point), the NXDetector.getAdditionalPluginList() are not added as NXdata groups. If doing a scan where file
			...
			nexusTree -> nexusTree.getData().isDetectorEntryData = false
		);
		return detectorData;
	}
