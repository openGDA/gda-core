...
@since: GDA 9.23
@since: GDA 9.35 added on, off switch
'''
...
class Metadata:
	...
	def addScalar(self, device_name, field_name, field_value, unit=None):
		...
		'''
		self._get_metadata_instance().addScalar(device_name, field_name, field_value, unit)
	...
	def addScannable(self, device_name, scannable):
		...
		'''
		self._get_metadata_instance().addScannable(device_name,  scannable)
	...
	def addPV(self, device_name, field_name, pv_name, unit=None):
		...
		'''
		self._get_epics_metadata_instance().addPV(device_name, field_name, pv_name, unit)
	...
	def addLink(self, device_name, field_name, link_path, file_name=None):
		...
		'''
		self._get_metadata_instance().addLink(device_name, field_name, file_name, link_path)
	...
	def rm(self, device_name, field_name):
		...
		"""
		self._get_metadata_instance().remove(device_name, field_name)
	...
	def disable(self, *device_names):
		...
		for device_name in device_names:
			self._get_metadata_instance().disable(device_name)
	...
	def ll(self, *args):
		...
		"""
		if not self.isOn():
			print("metadata is disabled or switched off. Please enable it by 'meta.on()'")
			return
		if len(args) == 0:
			self._get_metadata_instance().list(True)
		else:
			for each in args:
				if isinstance(each, str):
					self._get_metadata_instance().display(each, True)
				else:
	...
	def ls(self, *args):
		...
		if not self.isOn():
			...
			return
		if len(args) == 0:
			self._get_metadata_instance().list(False)
		else:
			for each in args:
				if isinstance(each, str):
					self._get_metadata_instance().display(each, False)
				else:
					raise ValueError("Input must be name or names of metadata devices in String!")

	def clear(self):
		'''
		clear all user added metadata only.
		'''
		self._get_metadata_instance().clear()

	def on(self):
		''' enable all metadata collection without changing metadata device configurations
		'''
		from org.eclipse.scanning.device import CommonBeamlineDevicesConfiguration
		CommonBeamlineDevicesConfiguration.getInstance().setEnabled(True)

	def off(self):
		''' disable all metadata collection without changing metadata device configurations
		'''
		from org.eclipse.scanning.device import CommonBeamlineDevicesConfiguration
		CommonBeamlineDevicesConfiguration.getInstance().setEnabled(False)

	def isOn(self):
		''' check if all metadata collection is enabled or not
		'''
		from org.eclipse.scanning.device import CommonBeamlineDevicesConfiguration
		return CommonBeamlineDevicesConfiguration.getInstance().isEnabled()

	def __initialize_common_device_configuration(self):
		if not self.initialized:
			from org.eclipse.scanning.device import CommonBeamlineDevicesConfiguration
			config = CommonBeamlineDevicesConfiguration.getInstance()
			if not config.isLaboratoryConfig():
				config.setEnforceMandatoryDeviceNames(
					installation.isLive() and get_machine_state() in [USER, SPECIAL]
				)
			self.initialized = True

	def _get_metadata_instance(self):
		self.__initialize_common_device_configuration()
		return NexusMetadataUtility.INSTANCE

	def _get_epics_metadata_instance(self):
		self.__initialize_common_device_configuration()
