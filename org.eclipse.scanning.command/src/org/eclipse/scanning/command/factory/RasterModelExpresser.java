...
class RasterModelExpresser extends PyModelExpresser<RasterModel> {
	...
	String pyExpress(RasterModel model, Collection<IROI> rois, boolean verbose) throws Exception {
		return new StringBuilder("grid(")
				...
				.append(formatValue(model.getBoundingBox().getFastAxisStart())+", ")
				.append(formatValue(model.getBoundingBox().getSlowAxisStart())+"), ")
				.append((verbose?"stop=":"")+"(")
				.append(formatValue(model.getBoundingBox().getFastAxisEnd())+", ")
				.append(formatValue(model.getBoundingBox().getSlowAxisEnd())+"), ")
				.append((verbose?"step=":"")+"(")
