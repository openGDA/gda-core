...
public class ProfileManagerPage extends WizardPage {
	...
	public ProfileManagerPage(String experimentId) {
	...
	public void createControl(final Composite parent) {
		...
		GridDataFactory fatButtonGridData = GridDataFactory.swtDefaults().hint(SWT.DEFAULT, 60)
	...
	private List<EditableWithListWidget> getProfilesForDriver(String driverName) {
		return getExperimentService().getDriverProfileNames(driverName, experimentId).stream()
				.map(name -> getDriverModel(name, driverName)).collect(Collectors.toList());
	}

	private DriverModel getDriverModel(String modelName, String driverName) {
		this.driverName = driverName;
		return getExperimentService().getDriverProfile(driverName, modelName, experimentId);
	}

	private void deleteProfile(EditableWithListWidget bean) {
		if (MessageDialog.openConfirm(getShell(), "Delete profile",
				"Are you sure you want to delete profile '" + bean.getLabel() + "'?")) {
