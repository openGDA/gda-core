...
public final class ManualSessionBeanComposite extends Composite {
	private static final Logger logger = LoggerFactory.getLogger(ManualSessionBeanComposite.class);

	private NumberBox sampleStorageTemperature;
	private ManualSampleFieldComposite measurements;
	private GridData layoutData;

	public ManualSessionBeanComposite(Composite parent, int style, final RichBeanEditorPart editor) {
		super(parent, style);
		...
		btnQueueExperiment.addSelectionListener(new SelectionAdapter() {
			private RemoteCommandRunner runner  = Finder.find("ManualRunner");
			@Override
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					logger.error("Couldn't run Manual experiment", ex);
					UIHelper.showError("Couldn't run experiment", ex);
				}
			}
		});
		measurements = new ManualSampleFieldComposite(this, SWT.NONE, editor);
