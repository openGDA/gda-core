 ...
 * 
 * In addition to the standard single value the box can save value as a list of values
 * or an iteration.
 * 
 * @author fcp94556
 *
...
public class RangeBox extends NumberBox implements IRangeWidget {
	...
	/**
	 * Create the composite RangeBox for use in GDA UI.
	 * 
	...
	protected void checkValue(String txt) {
		...
		if (!isList&&!isRange) {
			...
			if (isEditable()) {
				if (black==null) black = getDisplay().getSystemColor(SWT.COLOR_BLACK);
				if (!black.isDisposed()) text.setForeground(black);
			} else {
				if (grey==null) grey = getDisplay().getSystemColor(SWT.COLOR_DARK_GRAY);
				if (!grey.isDisposed()) text.setForeground(grey);
			}
		}
	}
	...
	private boolean checkBounds(final String value) {
		...
		for (Number val : dbls) {
			if (!isValidBounds(val.doubleValue())) {
				...
				setTooltipOveride("The range '"+value+"' would give the value '"+val+"' which is out of bounds.");
				return false;
			}
		}
		return true;
	...
	public String getValue() {
		...
		if ("".equals(txt.trim())) return null;
