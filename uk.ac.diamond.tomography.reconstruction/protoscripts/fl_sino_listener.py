...
#locate or create the output folders
   ...
   chunkprogram="image_vslice_tiff.q"
   chunkflags="-O %s -i %s -o %s -w %i -l %i -z %i  -Z %i  -s %i -p %i -b %i %s -S %s -I %s -T %i -R %i %s " % (outfmt,indir,outdir,wd,chunk_ht,interval,lt,nsegs,nperseg,bytes,existflag,settingsfolder,infmt,cropleft,cropright,idxflag) 
...
""") % settingsfolder
   ...
   try:
         ...
         if (vflag):
            ...
            print qenviron
         if (Wflag):
            thispid=os.spawnlpe(os.P_WAIT,"qsub","qsub", "-e",settingsfolder, "-o", settingsfolder, "-q",myqueue,"-N",jobname,"-hold_jid",mywait,"-cwd","-pe","smp","4","-t","%i-%i" % (firstchunk,lastchunk),chunkscript, qenviron)
         else:
            thispid=os.spawnlpe(os.P_WAIT,"qsub","qsub", "-e",settingsfolder, "-o", settingsfolder, "-q",myqueue,"-N",jobname,"-cwd","-pe","smp","4","-t","%i-%i" % (firstchunk,lastchunk),chunkscript, qenviron)
   ...
   try:
         ...
         thispid=os.spawnlpe(os.P_WAIT,"qsub","qsub","-e",settingsfolder,"-o",settingsfolder,"-q",myqueue,"-hold_jid",jobname,"-N",finishname,finishscript,qenviron)
         print ("return value was %s" % thispid)
