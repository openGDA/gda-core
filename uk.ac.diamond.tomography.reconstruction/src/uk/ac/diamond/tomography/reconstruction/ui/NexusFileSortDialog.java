...
/**
 * A dialog that allows us to set the nexus path that
 * will be used to sort nexus elements e.g. /entry1/entry_identifier
 */
public class NexusFileSortDialog extends SelectionDialog {
	 ...
	 * @param initialPath	an initialPath to set of <code>null</code> if not specified
	 * @param ruPaths	an array of recently used Paths to populate the combo box or <code>null</code>
	 */
	public NexusFileSortDialog(Shell parentShell, String initialPath, String[] ruPaths) {
		super(parentShell);
		setTitle("NeXus File Sorting");
		if (initialPath != null) this.initialPath = initialPath;
		if (ruPaths != null)this.recentlyUsedPaths = ruPaths;
	}
	...
	private void initializePaths() {
		if (nexusPathCombo != null) {
			...
			if (initialPath != null) nexusPathCombo.setText(initialPath);
		}
	}
	...
	/**
	 * Returns the sortPath
	 * @return the sortPath string or <code>null</code> if none was set
	 */
