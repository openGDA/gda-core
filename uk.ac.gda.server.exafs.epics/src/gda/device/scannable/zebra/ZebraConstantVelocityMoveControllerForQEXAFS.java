// Kept for reference only - to be deleted once QEXAFS is working
///*-
...
//import gda.device.zebra.ContinuousScannable;
//import gda.device.zebra.ZebraConstantVelocityMoveController;
//import gda.device.zebra.ZebraMotorInfoProvider;
...
//
//import gda.device.zebra.ZebraCaptureInputStreamCollection;
//
//
//public class ZebraConstantVelocityMoveControllerForQEXAFS extends ZebraConstantVelocityMoveController {
...
//				pcGateStart = start;
//				pcGateWidth = width + getAccelerationDistance();
//				double desiredSpeed = Math.abs(start - end) / getTotalTime();
...
//				requiredSpeed = Math.abs(start - end)/getTotalTime();
//				if (requiredSpeed > 100){
//					logger.error("Speed greater than max");
//				}
//				
//				//scannableMotor.asynchronousMoveTo(pcGateStart - 20);
//				scannableMotor.asynchronousMoveTo(pcGateStart - (step>0 ? 1.0 : -1.0)*getAccelerationDistance());
//				
//				break;
...
//	}
//	
//	public double getAccelerationDistance() throws DeviceException{
//		double accelerationDistance = zebraMotorInfoProvider.distanceToAccToVelocity(requiredSpeed);
//		double minimumAccelerationTime = Double.MAX_VALUE;
//		logger.info("accelerationDistance=" + accelerationDistance + " minimumAccelerationDistance=" + minimumAccelerationDistance);
//		if (accelerationDistance < minimumAccelerationDistance) {
//			// Since zebraMotorInfoProvider may use a different ACCL time to the actual motor and we can't get at the
//			// value zebraMotorInfoProvider uses anyway, use the actual motor ACCL time instead:
//			double timeToVelocity;
//			timeToVelocity = zebraMotorInfoProvider.getActualScannableMotor().getTimeToVelocity();
//			
//			double distanceAtVelocity = minimumAccelerationDistance - accelerationDistance;
//			double timeAtVelocity = distanceAtVelocity / requiredSpeed;
//			double totalTime = timeToVelocity + timeAtVelocity;
