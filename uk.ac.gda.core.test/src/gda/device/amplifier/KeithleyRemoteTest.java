...
/**
 * Test case for Keithley Remote amplifier
 */
public class KeithleyRemoteTest extends TestCase {
	/**
	 * Constructor
	 */
	public void testKeithley() {
		TestObjectAssistant toa = new TestObjectAssistant();
		toa.createRemoteObjects();

		Amplifier keithley = (Amplifier) Finder.getInstance().find("dummykeithley");
		...
		try {
			...
			assertEquals(+0.601E-03, keithley.getCurrentSuppressValue(), 0.0);
		...
		/*
		 * try { keithley.getStatus("H"); fail(); } catch (DeviceException de) { System.out.println(de.getMessage()); }
		 */
