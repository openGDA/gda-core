...
/** 
 * A named Spring-configurable {@link MonitorListener} for an EPICS PV of type {@link DBRType#INT}.
 * This listener stores a single integer data value which is updated via {@link MonitorEvent} from the EPICS PV by default,
 ...
 * 
 */
public class EpicsIntegerDataListener extends EpicsPVListener {

	protected int value;
	private Logger logger=LoggerFactory.getLogger(EpicsIntegerDataListener.class);
	
	@Override
	public void disablePoll() {
		if (pvchannel != null) {
			try {
				pvmonitor = pvchannel.addMonitor(DBRType.INT, pvchannel.getElementCount(), Monitor.VALUE, this);
				setPoll(false);
	...
	public Integer getValue() {
		if (isPoll()) {
			try {
				return controller.cagetInt(pvchannel);
