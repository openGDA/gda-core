...
public class WorkspaceResourceDialog extends ElementTreeSelectionDialog implements ISelectionStatusValidator {
	public static IContainer[] openFolderSelection(Shell parent, String message, boolean allowMultipleSelection,
	...
	public static IFile[] openFileSelection(Shell parent, String message, boolean allowMultipleSelection,
			Object[] initialSelection, List<ViewerFilter> viewerFilters) {
		WorkspaceResourceDialog dialog = new WorkspaceResourceDialog(parent, new WorkbenchLabelProvider(),
				new WorkbenchContentProvider());
		dialog.setAllowMultiple(allowMultipleSelection);
	...
	public static IFile openNewFile(Shell parent, String message, IPath suggestedPath, List<ViewerFilter> viewerFilters) {
		WorkspaceResourceDialog dialog = new WorkspaceResourceDialog(parent, new WorkbenchLabelProvider(),
				new WorkbenchContentProvider());
			...
			if (resource != null && resource.isAccessible()) {
				...
				if (suggestedPath.segmentCount() > 1) {
					...
					if (resource.getProject().isAccessible()) {
						...
						for (; !container.isAccessible() && container != resource.getProject(); container = container
								.getParent()) {
							// Do nothing
						}
				...
				if (suggestedPath != null) {
					String fileText = suggestedPath.isAbsolute() ? suggestedPath.removeFirstSegments(1).toString()
	...
	protected void createNewFolderControl(Composite parent) {
		...
		newFolderButton.setText("Create New Folder");
	...
	protected void createFileControl(Composite parent) {
		...
		fileLabel.setText("Create New File");
	...
	public IStatus validate(Object[] selectedElements) {
		...
		return enableOK ? new Status(IStatus.OK, "org.eclipse.emf.common.ui", 0, "", null) : new Status(IStatus.ERROR,
