...
public class MythenDetectorImpl extends DetectorBase implements Mythen, InitializingBean {
	...
	/**
	 * The Mythen client that actually interacts with the Mythen controller
	 * hardware.
	 */
	...
	/**
	 * The converter that converts raw Mythen data (channel and count) to
	 * processed data (angle and count).
	 */
	protected DataConverter dataConverter;
	
	/**
	 * The directory in which Mythen data files are saved.
	 */
	...
	public void setDetectorID(String detectorID) {
		...
		try {
			...
			throw new RuntimeException("Couldn't create NumTracker for Mythen detector", e);
		}
	}
	 ...
	 * 
	 * @param mythenClient the Mythen client
	 */
	 ...
	 * 
	 * @param deltaScannable the delta scannable
	 */
	...
	/**
	 * Sets the data directory into which Mythen data files will be written.
	 * 
	...
	public synchronized File getDataDirectory() {
		...
		// ensure do not use cached dataDirectory as it can be changed by setSubdirectory("subdir").
			dataDirectory = new File(PathConstructor.createFromDefaultProperty());
		//}
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (dataConverter == null) {
			throw new IllegalStateException("You have not set a data converter; this is needed for converting raw data to processed data");
		}
	}
	...
	private void _multi(Trigger trigger, int numFrames, double delayBeforeFrames, double exposureTime, double delayAfterFrames) throws DeviceException {
		...
		AcquisitionParameters params = new AcquisitionParameters.Builder()
			.frames(numFrames)
			.delayBeforeFrames(new BigDecimal(delayBeforeFrames))
			.exposureTime(new BigDecimal(exposureTime))
			.delayAfterFrames(new BigDecimal(delayAfterFrames))
			.filename(baseFilename.getAbsolutePath())
			.startIndex(1)
			.trigger(trigger)
			.build();
	...
	public void gated(int numFrames, int numGates) throws DeviceException {
		...
		AcquisitionParameters params = new AcquisitionParameters.Builder()
			.delayBeforeFrames(ZERO)
			.delayAfterFrames(ZERO)
			.exposureTime(ZERO)
			.frames(numFrames)
			.gating(true)
			.gates(numGates)
	...
	/**
	 * gated multiple frames collection - one frame per file - 
	 * where Mythen detector controls the frame number increment starting from 0.
	 * It launches only single textclient to collect multiple frames
	 * 
	 * @param numFrames
	 * @param numGates
	 * @param scanNumber
	...
	public void gated(final int numFrames, int numGates,long scanNumber) throws DeviceException {
		...
		AcquisitionParameters params = new AcquisitionParameters.Builder()
			...
			.filename(rawFile.getAbsolutePath())
			.trigger(Trigger.NONE)
			.startIndex(0) // comment out to save to single file, remove comment to save to multiple files
			.build();
	...
	/**
	 * post processes of multiple frames collection and plotting them.
	 * 
	...
	/**
	 * gated multiple or single frame collection - one or more frames per file -
	 * where GDA controls the collection number increment.
	 * 
	 * @param numFrames
	 * @param numGates
	 * @param scanNumber
	...
	public void gated(final int numFrames, int numGates,long scanNumber, int collectionNumber) throws DeviceException {
		// must set scanNumber when operate outside GDA Scan command, e.g. in a for loop. This is normally set atScanStart() in a scan
		this.scanNumber=scanNumber;
		// must set collection number when operate outside GDA Scan command, e.g. in a for loop. This is normally initialised atScanStart() in a scan
		this.collectionNumber=collectionNumber;
