...
public class MythenDetectorImpl extends DetectorBase implements Mythen, InitializingBean {
	...
	protected void afterCollectData(File rawFile, int num) {
		...
		status = IDLE;
		plot(processedData, num);
	}

	public void plot(MythenProcessedDataset processedData, int num) {
		...
		DataSet countsDataset = new DataSet("count", counts);
		if (num == 1) {
			Plotter.plot("Mythen", channelsDataset, countsDataset);
		} else {
			Plotter.plotOver("Mythen", channelsDataset, countsDataset);
		}
	}
