...
class I20XasScan(XasScan):
    ...
    #       # XAS / XANES room temperature sample stage 
            ...
            # ensure that the mono is stopped if an abort occurs while its moving
        ...
        try:
            while True:
                ...
                try:
                    ...
                    ic = self.finder.find("ionchambers")
                    if ic != None:
                        ic.stop()
                        ic.clearFrameSets()
...
#            self.jython_mapper.twodplotter.setZ_colName(self.finder_mapper.xmapMca.getExtraNames()[0])
            ...
            self.jython_mapper.twodplotter.setxStart(beanGroup.getScan().getXesInitialEnergy())
            self.jython_mapper.twodplotter.setxStop(beanGroup.getScan().getXesFinalEnergy())
            self.jython_mapper.twodplotter.setxStep(beanGroup.getScan().getXesStepSize())
            self.jython_mapper.twodplotter.setyStart(beanGroup.getScan().getMonoInitialEnergy())
            self.jython_mapper.twodplotter.setyStop(beanGroup.getScan().getMonoFinalEnergy()) 
            self.jython_mapper.twodplotter.setyStep(beanGroup.getScan().getMonoStepSize())
